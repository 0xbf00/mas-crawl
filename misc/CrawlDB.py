...
def log_error(err: str) -> bool:
    '''
    Logs an error to stderr and returns False, to allow
    callers to simply return log_error(...)
    :param err: The error string to write to stderr
    :return: False
    '''
...
def db_read_config(config_file : str = DB_CONFIG_FILE) -> dict:
    '''
    Read config for a database. The config format is simply json. Two keys are expected:
    database_name and database_user
    :param config_file: The path to the database configuration file.
    :return: dictionary containing config
    '''
    response = {}

    with open(config_file) as config:
        response = json.load(config)
...
def db_exists_table(connection : object, table_name : str) -> bool:
    ...
    :return: True, if the table name exists, else False
    '''
    c = connection.cursor()
...
def db_create_table(connection: object, table_sql: str) -> bool:
    '''
    Create a table with the specified SQL. This is mainly just a wrapper
    around internal functions and does nothing special. It's main purpose is
    to provide cleaner code.

    :param connection: Connection to the database
    :param table_sql: SQL statement(s) to execute
...
class MacCrawl:
    '''
    Simple class modelling a crawl of the Mac App Store.
    The database entry that goes with this has the following declaration:
